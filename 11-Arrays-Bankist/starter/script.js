...
const overalBalance2 = accounts
  ...
  .reduce((acc, mov) => acc + mov, 0);
console.log('overalBalance with flatMap:', overalBalance);
...
//     acc.push(breed.activities.filter(act => act !== 'swimming'));
// }, []);
...
// 6. solved, but small doubt.

// const weightsSum = breeds.reduce(
//   (acc, breed) => (acc += breed.averageWeight),
//   0,
// );

// console.log(weightsSum / breeds.length > 10);
...
//   console.log(
//     `Is ${breed.breed} dog is active ?\nAns:- ${breed.activities.length >= 3}`,
//   );
// });

console.log(breeds.some(breed => breed.activities.length >= 3));

// BONUS: not understood properly.

const allFetchWeights = breeds
...
const groupedActivity = Object.groupBy(accounts, acc => {
  ...
  if (movCount >= 8) return 'very_active';
...
console.log(
  'All deposits',
  accounts.flatMap(acc => acc.movements).filter(mov => mov > 0),
);
...
// Prefixed ++ operator.
let a = 10;
// console.log(a++); this is log the value fist, then increase the value.
console.log(++a);
...
console.log(
  '1. The dogs recommended food keys add with this key(recFood) and recommended food calculated with this formal(weight ** 0.75 * 28):',
  dogs,
);
...
console.log('2. sarahDog', sarahDog);
console.log(
  `2. the sarah dog is eating too ${sarahDog.curFood > sarahDog.recFood ? 'much' : 'little'}.`,
);

// 3.
const ownersTooMuch = dogs.reduce((owners, dog) => {
  dog.curFood > dog.recFood && owners.push(...dog.owners);
  return owners;
}, []);

const ownersTooLittle = dogs.reduce((owners, dog) => {
  dog.curFood < dog.recFood && owners.push(...dog.owners);
  return owners;
}, []);

console.log('3. The too much eating dogs owners', ownersTooMuch);
console.log('3. The too little eating dogs owners', ownersTooLittle);

// 4.
const LogOwners = (owners, isMuchEating) =>
  console.log(
    `4. ${owners.join(' and ')}'s dogs eat too ${isMuchEating ? 'much' : 'little'}!`,
  );

LogOwners(ownersTooMuch, true);
LogOwners(ownersTooLittle, false);

// 5.
console.log(
  `5. Is any dog is eating perfect ?, Ans: ${dogs.some(dog => dog.curFood === dog.recFood)}`,
);

// 6.
console.log(
  `6. Is all dogs are eating OKAY amount of food: ${dogs.every(dog => dog.curFood > dog.recFood * 0.9 && dog.curFood < dog.recFood * 1.1)}`,
);

// 7.
const okayAmountFoodEatingDogs = dogs.filter(
  dog => dog.curFood > dog.recFood * 0.9 && dog.curFood < dog.recFood * 1.1,
);

console.log('The OKAY amount food eating dogs', okayAmountFoodEatingDogs);
...
console.log('8. groupedDogs', groupedDogs);

// 9.
const groupedByOwnersLength = Object.groupBy(
  dogs,
  dog => `${dog.owners.length}-owners`,
);
console.log('9. Grouped dogs based on owners length', groupedByOwnersLength);

// 10.
const sortByRecFood = dogs.toSorted((a, b) => a.recFood - b.recFood);

console.log('10. sorted the dogs by recommended Food', sortByRecFood);
