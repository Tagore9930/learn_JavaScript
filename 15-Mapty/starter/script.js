...
class App {
  ...
  _moveToPopup(e) {
    ...
    if (!workoutEle?.dataset?.id)
      return console.error('The workoutEle dataset id is not found');

    const workout = this.#workouts.find(
      work => work.id == workoutEle.dataset.id,
    );

    if (!workout) console.error('The workout list item is not found.');
  ...
  _getLocalStorage() {
    const data = JSON.parse(localStorage.getItem('maptyWorkouts'));
