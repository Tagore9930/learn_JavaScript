...
Use this API: // Because the above api is not working now
https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${lat}&longitude=${lng}.
...
get3Countries('india', 'us', 'pakistan');
*/
...
const loadAll = async function (imgArr = []) {
  try {
    if (!imgArr && imgArr?.length == 0)
      throw new Error('Images array is not passed');

    const promiseImgs = imgArr?.map(async imgPath => {
      return await createImage(imgPath);
    });

    console.log(promiseImgs);

    const domImgs = await Promise.all(promiseImgs);
    console.log(domImgs);

    domImgs.map(img => img.classList.add('parallel'));
  } catch (err) {
    console.log(err);
  }
};
